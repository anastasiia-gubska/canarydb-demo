...
canarydemo/
├── apps/
│   ├── v1/app.go         # Legacy App (reads/writes legacy full_name column)
│   └── v2/app.go         # Transition App (writes to all 3 columns, reads from 2 new columns)
│   └── v3/app.go         # Final App (reads/writes new columns)
├── Dockerfile            # Multi-stage build for v1, v2 and v3
...
```
### 2. Populate DB with data and watch
```
kubectl exec -n stateful-app-demo deploy/postgres -it -- psql -U postgres -d testdb < db_schema/000_init_schema.sql
watch -n 2 "sudo kubectl exec -n stateful-app-demo deploy/postgres -- psql -U postgres -d testdb -c 'SELECT * FROM users;'"
```
...
./start_demo.sh
```
- Stable (v1): http://localhost:8080
- Canary (v2): http://localhost:8081 (won't be enabled)
